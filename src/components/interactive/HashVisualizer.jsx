...
export const HashVisualizer = () => {
  ...
  useEffect(() => {
    ...
    const fake = ((h ^ h >>> 16) >>> 0).toString(16).padStart(8,'0') + input.length.toString(16) + "f2a9";
  ...
  return (
    <div className={`${styles.glassCard} p-6 rounded-2xl`}>
      ...
      <p className="text-xs text-slate-400 mt-4">区块链不可篡改的基石。修改任意一个字符，哈希指纹都会完全改变（雪崩效应）。</p>
    </div>
  );
};