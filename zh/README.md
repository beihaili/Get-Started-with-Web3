# Get Started With Web3 — 中文教程

[![GitHub stars](https://img.shields.io/github/stars/beihaili/Get-Started-with-Web3)](https://github.com/beihaili/Get-Started-with-Web3/stargazers)
[![GitHub forks](https://img.shields.io/github/forks/beihaili/Get-Started-with-Web3)](https://github.com/beihaili/Get-Started-with-Web3/network/members)
[![GitHub issues](https://img.shields.io/github/issues/beihaili/Get-Started-with-Web3)](https://github.com/beihaili/Get-Started-with-Web3/issues)
[![GitHub license](https://img.shields.io/github/license/beihaili/Get-Started-with-Web3)](https://github.com/beihaili/Get-Started-with-Web3/blob/main/LICENSE)
[![Contributors](https://img.shields.io/github/contributors/beihaili/Get-Started-with-Web3)](https://github.com/beihaili/Get-Started-with-Web3/graphs/contributors)

** 中文世界最系统的比特币与 Web3 开源学习平台。** 11 个模块、62 讲体系化教程，覆盖从 Web3 入门、以太坊智能账户、比特币底层原理到 DeFi、L2、DAO 和 Builder 实战的完整知识体系。

** 在线学习平台（推荐）：** https://beihaili.github.io/Get-Started-with-Web3/

---

## 学习路径

建议按顺序学习：** 先完成 Web3 快速入门 ** → ** 理解以太坊与智能账户 ** → ** 再深入比特币技术、DeFi、L2 和 Builder 实战 **。

有编程基础的开发者可以直接进入以太坊与智能账户、Web3 Builder 实战或比特币技术系列。

---

## 1. Web3 快速入门（7 讲）

从零开始，无需编程基础，7 讲掌握 Web3 核心操作。

| 讲次 | 主题 | 关键内容 |
|------|------|----------|
| 01 | [创建你的第一个 Web3 身份](Web3QuickStart/01_FirstWeb3Identity/README.md) | MetaMask 安装、地址创建、助记词备份 |
| 02 | [进行你的第一笔 Web3 交易](Web3QuickStart/02_FirstWeb3Transaction/README.md) | 转账操作、Gas 费、交易确认机制 |
| 03 | [构建你的第一个 Web3 DApp](Web3QuickStart/03_FirstWeb3Dapp/README.md) | 钱包连接、智能合约交互 |
| 04 | [有用的 Web3 站点](Web3QuickStart/04_UsefulWeb3Sites/README.md) | 区块浏览器、DeFi 工具、信息聚合站点 |
| 05 | [发行你的第一个代币](Web3QuickStart/05_LaunchYourFirstToken/README.MD) | Pump.fun、Solana 钱包、Meme 币风险 |
| 06 | [Web3 安全防护与防骗指南](Web3QuickStart/06_Web3Security/README.md) | 常见骗局识别、资产保护、安全最佳实践 |
| 07 | [Web3 交易所入门](Web3QuickStart/07_Web3CEX/README.MD) | CEX 入金、提币、链选择和交易安全 |

---

## 2. 以太坊与智能账户（3 讲）

理解 Pectra、Fusaka 后的以太坊路线，以及账户抽象如何改变钱包和 DApp 体验。

| 讲次 | 主题 | 关键内容 |
|------|------|----------|
| 01 | [Pectra 与 Fusaka 后的以太坊](EthereumSmartAccounts/01_EthereumAfterPectra/README.md) | EIP-7702、Blob、PeerDAS、L2 扩容路线 |
| 02 | [账户抽象与智能钱包](EthereumSmartAccounts/02_AccountAbstractionAndSmartWallets/README.md) | ERC-4337、Bundler、Paymaster、Session Key |
| 03 | [UserOperation 模拟器与账户抽象实践](EthereumSmartAccounts/03_UserOperationSimulator/README.md) | UserOperation、EntryPoint、Paymaster、EIP-7702 委托 |

---

## 3. 比特币技术入门（21 讲）

按 **5 层技术架构 ** 组织，从密码学基础到生态应用。每一讲都包含原理讲解 + 生活化类比 + 配套代码。

> 完整的架构图、学习路径和配套工具请查看：[技术架构总览](GetStartedWithBitcoin/README.md)

### 第一层：密码学基础层

| 讲次 | 主题 | 关键内容 |
|------|------|----------|
| 01 | [密码学基础](GetStartedWithBitcoin/01_Cryptography/README.MD) | 哈希函数、数字签名、椭圆曲线、HD 钱包 |
| 02 | [比特币概述](GetStartedWithBitcoin/02_Overview/README.MD) | 比特币本质、去中心化设计哲学 |

### 第二层：数据层

| 讲次 | 主题 | 关键内容 |
|------|------|----------|
| 03 | [比特币交易基础](GetStartedWithBitcoin/03_BitcoinTx/README.MD) | UTXO 模型、交易结构与验证 |
| 04 | [多重签名交易](GetStartedWithBitcoin/04_MultiSig/README.MD) | 多签机制、安全模型 |
| 05 | [隔离见证 (SegWit)](GetStartedWithBitcoin/05_SegWit/README.MD) | 交易数据重组、容量优化 |
| 06 | [Taproot 升级详解](GetStartedWithBitcoin/06_Taproot/README.MD) | Schnorr 签名、MAST、隐私优化 |
| 07 | [高级交易应用](GetStartedWithBitcoin/07_AdvancedTransactions/README.MD) | 时间锁、复杂脚本 |
| 08 | [区块链数据结构](GetStartedWithBitcoin/08_DataStructure/README.MD) | 区块结构、默克尔树 |

### 第三层：网络层

| 讲次 | 主题 | 关键内容 |
|------|------|----------|
| 09 | [比特币核心节点](GetStartedWithBitcoin/09_BitcoinCore/README.MD) | 节点运行、网络参与 |
| 10 | [P2P 网络协议](GetStartedWithBitcoin/10_P2PProtocol/README.MD) | 节点发现、数据传播与同步 |
| 11 | [网络安全](GetStartedWithBitcoin/11_NetworkSecurity/README.MD) | 攻击类型与防护、隐私保护 |

### 第四层：共识层

| 讲次 | 主题 | 关键内容 |
|------|------|----------|
| 12 | [工作量证明与挖矿](GetStartedWithBitcoin/12_ProofOfWork/README.md) | 挖矿算法、硬件演进、矿池机制 |
| 13 | [难度调整算法](GetStartedWithBitcoin/13_DifficultyAdjustment/README.MD) | 动态难度调整、经济激励平衡 |
| 14 | [分叉机制与 BIP 流程](GetStartedWithBitcoin/14_ForksBIPs/README.MD) | 软分叉 vs 硬分叉、协议升级 |

### 第五层：应用层

| 讲次 | 主题 | 关键内容 |
|------|------|----------|
| 15 | [比特币钱包](GetStartedWithBitcoin/15_BitcoinWallet/README.MD) | 钱包类型、密钥管理 |
| 16 | [比特币 RPC 应用开发](GetStartedWithBitcoin/16_BitcoinRPC/README.MD) | RPC 接口、系统集成 |
| 17 | [比特币低费率广播工具](GetStartedWithBitcoin/17_BitcoinLowFeeBroadcast/README.MD) | 费用优化、实用工具开发 |
| 18 | [Bitcoin Script 脚本系统](GetStartedWithBitcoin/18_BitcoinScript/README.MD) | 栈式脚本语言、条件支付 |
| 19 | [比特币扩容与治理](GetStartedWithBitcoin/19_BitcoinGovernance/README.MD) | Layer 2、闪电网络、治理机制 |
| 20 | [Ordinals 与生态创新](GetStartedWithBitcoin/20_Ordinals/README.MD) | 铭文、BRC-20、NFT |
| 21 | [DeFi 跨链](GetStartedWithBitcoin/21_DeFiCrossChain/README.MD) | 跨链协议、去中心化金融 |

---

## 4. Web3 思考集

| 主题 | 简介 |
|------|------|
| [Web3 原则](Web3Thoughts/01_Principles/README.MD) | Web3 的核心理念与价值观 |
| [为什么区块链是必须的？](Web3Thoughts/02_WhyBlockchainIsNecessary/README.MD) | 《主权个人》读书笔记 |
| [区块链彩蛋解读](Web3Thoughts/03_TheCoolestTransactionOnBitcoin/README.MD) | b10c...eb5d 交易中的隐藏彩蛋 |

## 5. 进阶专题模块

| 主题 | 简介 |
|------|------|
| [Web3 Builder 实战](Web3BuilderLab/01_DeployERC20/README.md) | 从部署代币、构建 DApp、钱包和 SIWE 实验到合约安全实战 |
| [DeFi 深度解析](DeFiDeepDive/01_CoreConcepts/README.md) | AMM、借贷、稳定币和 DeFi 风险 |
| [跨链与 Layer 2 深度解析](L2CrossChain/01_WhyScaling/README.md) | Rollup、L2 生态、跨链桥、风险模拟和实操指南 |
| [DAO 与去中心化治理](DAOGovernance/01_WhatIsDAO/README.md) | DAO 机制、工具、案例和治理挑战 |

## 6. 其它学习资源

| 主题 | 简介 |
|------|------|
| [以太坊学习指南](其它学习资源整理 / Etherum/README.MD) | 以太坊生态入门资源 |
| [DeFi 学习指南](其它学习资源整理 / DeFi/README.MD) | DeFi 协议与工具 |

## 7. Web3 工作机会

| 主题 | 简介 |
|------|------|
| [工作机会和面试技巧](Web3WorkOpportunities/README.md) | Web3 求职方向与准备 |

## 8. 研究报告

| 主题 | 简介 |
|------|------|
| [BSC 链上攻击分析](OurResearch/Search01_BscAttack/README.md) | BSC 跨链桥攻击事件深度分析 |

---

## 参与贡献

Web3 的世界需要你的声音和智慧。无论你是区块链专家还是刚入门的新人，每一份贡献都很宝贵。

### 贡献方式

| 类型 | 说明 |
|------|------|
| ** 内容审核 ** | 审查教程的技术准确性，提交勘误 |
| ** 教程补充 ** | 添加代码示例、图表、实际案例 |
| ** 英文翻译 ** | 帮助翻译教程，扩大受众覆盖 |
| ** 代码示例 ** | 编写可运行的配套代码 |
| ** 平台开发 ** | 改进 React 学习平台功能 |

### 贡献流程

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/amazing-contribution`)
3. 提交更改 (`git commit -m 'Add some amazing contribution'`)
4. 推送到分支 (`git push origin feature/amazing-contribution`)
5. 创建 Pull Request

### 质量标准

- 技术内容经过验证
- 复杂概念用简单语言解释
- 代码示例可运行
- 格式与现有教程一致

> 详细贡献说明和章节认领请查看 [比特币技术系列贡献指南](GetStartedWithBitcoin/README.md#- 如何参与贡献)

---

## 联系我们

- **Twitter**: [@bhbtc1337](https://twitter.com/bhbtc1337)
- ** 微信交流群 **: [填表加入](https://forms.gle/QMBwL6LwZyQew1tX8)
- **GitHub Issues**: [提交问题](https://github.com/beihaili/Get-Started-with-Web3/issues)
- ** 在线学习 **: [GitHub Pages](https://beihaili.github.io/Get-Started-with-Web3/)

## 贡献者

感谢所有为这个项目做出贡献的人！

<a href="https://github.com/beihaili/Get-Started-with-Web3/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=beihaili/Get-Started-with-Web3&t=1717826094" />
</a>

## 许可证

本项目采用 [MIT 许可证](https://github.com/beihaili/Get-Started-with-Web3/blob/main/LICENSE)。

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=beihaili/Get-Started-with-Web3&type=Date)](https://star-history.com/#beihaili/Get-Started-with-Web3&Date)

---

<p align="center"><strong > 让我们一起构建最好的中文 Web3 与比特币技术教程！</strong></p>
