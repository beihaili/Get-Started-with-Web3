...
- [zkSync](#zksync)
- [StarkNet](#starknet)
- [2026 更新：Blob、PeerDAS 与 L2 风险分层](#2026-更新blobpeerdas-与-l2-风险分层)
- [各 L2 综合对比](# 各 - l2 - 综合对比)
- [总结](# 总结)
- [延伸阅读](# 延伸阅读)
...
## 2026 更新：Blob、PeerDAS 与 L2 风险分层

2024 年 Dencun 引入 Blob 后，L2 成本结构发生了根本变化。2025 年 Pectra 提高 Blob 容量，2025 年底 Fusaka 引入 PeerDAS，让 Blob 继续扩容变得更可持续。理解 2026 年的 L2，不能只看 TVL 和手续费，还要看数据可用性、排序器、证明系统和升级权限。

### Blob 经济：L2 为什么变便宜

Rollup 需要把交易数据发布到以太坊，确保任何人都能重建 L2 状态。过去很多数据放在 calldata 里，成本高；EIP-4844 引入 Blob 后，Rollup 获得了专门的数据通道。
...
Blob 降低的是“L1 数据发布成本”这一部分。Pectra 把 Blob 目标容量提高后，L2 在常态下更不容易因为数据空间紧张而费用飙升。但 Blob 仍然是市场资源，如果多个 Rollup 同时拥堵，费用依然会上升。

### PeerDAS：扩容不应牺牲节点可运行性

Fusaka 的 PeerDAS 重点解决“Blob 越多，全节点负担越重”的问题。它让节点通过数据可用性采样参与验证，不必每个节点都下载所有 Blob 数据。
...
- 普通节点的带宽和硬件压力不至于线性增长。
- L2 低费用更有机会长期维持。
- Rollup 的扩容路线更依赖以太坊 DA 能力，而不是单纯依赖中心化数据层。

### OP Superchain 与应用链趋势

Optimism 的 OP Stack 已经不只是 Optimism 主网本身，而是一套可复用的 Rollup 技术栈。Base、Zora、Mode、World Chain 等都属于这个路线。
...
- 升级、治理和基础设施尽量统一。
- 链之间未来可以有更顺滑的消息和资产流动。
- 应用可以选择成为一条专用链，而不是挤在同一个通用 L2 上。

这会带来一个新问题：用户看到的不再是“以太坊 vs 某个 L2”，而是“一组同技术栈链之间如何安全互操作”。

### Base：消费级 L2 的代表

Base 的特点不是技术最激进，而是分发能力强：

- Coinbase 用户入口带来法币和主流用户通道。
- OP Stack 让它继承 Superchain 生态能力。
- Farcaster、社交、创作者经济、Meme 和小额支付应用活跃。
- 没有原生代币，降低了“为了空投而交互”的单一叙事。

学习 Base 时要关注两个问题：

1. 它如何把中心化交易所用户导入链上自托管体验。
2. 它在排序器、治理、数据发布和收入分配上如何逐步去中心化。

### ZK Rollup：技术强，但生态和兼容性仍是挑战

zkSync、StarkNet、Scroll、Polygon zkEVM 等都属于 ZK 路线，但它们的开发体验、EVM 兼容性、证明成本和生态策略差异很大。

ZK Rollup 的优势：

- 有效性证明能更快给出状态正确性保证。
- 理论上提款等待期可以比 Optimistic Rollup 更短。
- 适合复杂计算、隐私、游戏和高频应用的长期扩展。

挑战：

- 证明系统复杂，审计和实现难度高。
- 开发者工具和 EVM 兼容性需要长期打磨。
- 生态冷启动比 OP Stack 路线更难。
- 升级密钥和中心化证明器仍是阶段性风险。

### Alt-DA、Validium 和 Rollup 不完全一样

很多低费链会使用以太坊以外的数据可用性方案，例如 Celestia、EigenDA、Avail 或自建 DAC。它们可能显著降低费用，但安全假设不同。

| 类型 | 数据放在哪里 | 优势 | 风险 |
|------|--------------|------|------|
| Rollup | 以太坊 L1 Blob / calldata | 继承以太坊 DA 安全 | 成本较高 |
| Validium | 链下 DA 或委员会 | 费用更低，吞吐更高 | 数据不可用时用户退出更难 |
| Optimium / Alt-DA Rollup | 第三方 DA 层 | 成本和扩展性折中 | 依赖额外 DA 网络安全 |

所以不要只看“手续费低”，要问：如果运营方作恶或 DA 层故障，用户能不能独立退出？

### 选择 L2 的风险框架

使用或研究 L2 时，可以按下面 7 个问题检查：

1. ** 数据可用性 **：交易数据发布到以太坊、第三方 DA，还是委员会？
2. ** 证明系统 **：欺诈证明或有效性证明是否真正上线？
3. ** 排序器 **：是否单一排序器？是否有去中心化路线？
4. ** 退出路径 **：用户是否可以强制退出到 L1？
5. ** 升级权限 **：合约是否可升级？多签或治理延迟多久？
6. ** 桥安全 **：官方桥和第三方桥分别承担什么风险？
7. ** 生态质量 **：TVL 是真实应用需求，还是短期激励堆出来的？
