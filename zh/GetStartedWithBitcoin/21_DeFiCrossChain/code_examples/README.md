...
本目录包含第 21 讲中所有代码示例的完整实现，按功能模块组织。

## 目录结构

### 📁 lightning_network/
Lightning Network 相关实现：
- `payment_channel.py` —— 支付通道核心实现。
- `routing_network.py` —— 路由网络和路径寻找算法。

### 📁 rgb_protocol/
RGB 协议智能合约实现：
- `rgb_token.py` —— RGB 代币和 NFT 合约实现。

### 📁 atomic_swaps/
原子交换和跨链技术：
- `atomic_swap.py` —— 原子交换 HTLC 实现和跨链桥接。
...
### 📁 applications/
实际应用场景实现：
- `lightning_payment_app.py` —— 完整的 Lightning 支付应用。
...
这些示例代码将帮助你深入理解比特币 DeFi 生态系统的技术内核和创新思路。
