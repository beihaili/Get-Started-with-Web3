...
export const Web3Oracle = () => {
  const [status, setStatus] = useState('idle'); // idle, signing, revealed
  const [fortune, setFortune] = useState(null);
  
  const fortunes = [
    { rarity: 'Common', color: 'text-slate-400', border: 'border-slate-500', bg: 'bg-slate-500/20', text: "今天的 Gas 费平平无奇，适合潜心学习 Solidity。" },
    { rarity: 'Uncommon', color: 'text-green-400', border: 'border-green-500', bg: 'bg-green-500/20', text: "牛市在心中。保持耐心，HODL 住手中的优质资产。" },
    { rarity: 'Rare', color: 'text-cyan-400', border: 'border-cyan-500', bg: 'bg-cyan-500/20', text: "链上数据显示今日适合交互。去尝试一个新的 DApp 吧！" },
    { rarity: 'Epic', color: 'text-purple-400', border: 'border-purple-500', bg: 'bg-purple-500/20', text: "灵感涌现！今天是你部署第一个智能合约的黄道吉日。" },
    { rarity: 'Legendary', color: 'text-yellow-400', border: 'border-yellow-500', bg: 'bg-yellow-500/20', text: "✨ 传说级运势！你可能会发现下一个 Alpha 项目的线索。" },
  ];
    ...
    <div className={`${styles.glassCard} p-8 rounded-2xl relative overflow-hidden flex flex-col items-center justify-center text-center min-h-[300px] group`}>
        ...
        <div className="space-y-6 animate-in fade-in zoom-in duration-500">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm max-w-xs mx-auto">
              利用你的钱包签名作为熵源 (Entropy)，<br/>生成今日专属的链上运势。
            </p>
          </div>
          <button 
      ...
      {status === 'revealed' && fortune && (
        <div className="space-y-6 animate-in zoom-in-95 duration-500 w-full">
          <div className={`inline-block px-3 py-1 rounded-full border ${fortune.border} ${fortune.bg} ${fortune.color} text-xs font-bold tracking-widest uppercase mb-2`}>
            {fortune.rarity} Drop
          </div>
